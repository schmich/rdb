...
app.controller('MainCtrl', function($scope, $http) {
  ...
  $scope.events = new EventSource('/events');
  $scope.events.addEventListener('message', function(e) {
    var message = JSON.parse(e.data);
    if ((message.event == 'breakpoint-hit') || (message.event == 'step-hit')) {
...
app.controller('SourceEditCtrl', function($scope, $http) {
  ...
  $scope.events.addEventListener('message', function(e) {
    var message = JSON.parse(e.data);
    if ((message.event == 'breakpoint-created') || (message.event == 'breakpoint-deleted')) {
      $scope.$apply(function() {
        updateBreakpoints();
      });
    }
